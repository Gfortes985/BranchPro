...
const DEFAULT_SERVER_HOST = "81.30.105.141";
const FALLBACK_SERVER_HOSTS = ["localhost:3000", "127.0.0.1:3000"] as const;

const normalizeHost = (host: string): string => host.trim().replace(/\/+$/, "");

const buildBaseUrlCandidates = (host: string): string[] => {
  ...
  // For this API deployments are often plain HTTP behind reverse-proxy/LAN,
  // so probe HTTP first to avoid noisy HTTPS ERR_CONNECTION_REFUSED in dev.
...
export default function App() {
  ...
  const [serverHost, setServerHost] = useState<string>(localStorage.getItem("bp_server_host") ?? DEFAULT_SERVER_HOST);
  const [baseUrl, setBaseUrl] = useState<string>(buildBaseUrlCandidates(serverHost)[0] ?? `https://${DEFAULT_SERVER_HOST}`);
  ...
  const saveServerHost = () => {
    const normalized = normalizeHost(serverHost);
    setServerHost(normalized);
    localStorage.setItem("bp_server_host", normalized);
    setServerOk(null);
    setToast("✅ Хост сервера сохранён");
  };
  ...
  async function checkServer() {
    ...
    try {
      ...
      const hostCandidates = [serverHost, ...FALLBACK_SERVER_HOSTS]
        .map(normalizeHost)
        .filter(Boolean)
        .filter((h, idx, arr) => arr.indexOf(h) === idx);

      const baseCandidates = hostCandidates.flatMap(buildBaseUrlCandidates);
      ...
      if (!found) {
        setServerOk(false);
        setToast(`❌ Сервер недоступен (${hostCandidates.join(", ")}). Проверь хост/порт, HTTPS/HTTP, CORS и прокси до API.`);
      }
    } finally {
      setBusy(false);
    }
    ...
    <div className="shell">
      ...
      <main className="main">
        ...
        <div className="content">
          ...
          {page === "settings" ? (
            <Card title="Сервер">
              <div className="label">Server host/IP</div>
              <div className="row" style={{ alignItems: "flex-end", flexWrap: "wrap" }}>
                <input
                  className="inp"
                  value={serverHost}
                  onChange={(e) => setServerHost(e.target.value)}
                  placeholder="81.30.105.141 или localhost:3000"
                  style={{ minWidth: 280 }}
                />
                <button className="btn" onClick={saveServerHost} disabled={busy || !serverHost.trim()}>
                  Сохранить хост
                </button>
              </div>
              <div className="muted mono" style={{ marginTop: 8 }}>{`${baseUrl}${apiPrefix}`}</div>
              ...
              <div style={{ marginTop: 12 }} className="muted">
                Приложение подбирает HTTPS/HTTP и префикс API автоматически (с fallback на localhost).
              </div>
